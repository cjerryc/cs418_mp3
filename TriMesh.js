 ...
 */
    ...
    */
    loadFromOBJ(fileText)
    {
        ...
        //Your code here
        let obj_strings = fileText.split('\n');      //separate into individual lines by newline

        for(let i=0; i < obj_strings.length; i++){   //for as long as file is not yet fully parsed
            let obj_string = obj_strings[i].split(/\s+/g);
            if(obj_string[0] == '#'){               //Comment to be printed to console
                let x=1;
                let comment = '';
                while(x < obj_string.length){
                    comment = comment + obj_string[x];
                    x++;
                }
                console.log(comment);
            }
            if(obj_string[0] == 'v'){           //append next 3 points to Vertex array
                this.numVertices++;
                for(var j=1; j<4; j++){
                    this.vBuffer.push(parseFloat(obj_string[j]));           //3 points added to global array
                    if(this.minXYZ[j-1] > parseFloat(obj_string[j])){                   //update minXYZ coordinates
                        this.minXYZ[j-1] = parseFloat(obj_string[j]);
                    }
                    if(this.maxXYZ[j-1] < parseFloat(obj_string[j])){                   //update maxXYZ coordinates
                        this.maxXYZ[j-1] = parseFloat(obj_string[j]);
                    }
                }
            }
            if(obj_string[0] == 'f'){           //append next 3 points to Face array
                this.numFaces++;
                for(var j=1; j<4; j++){
                    this.fBuffer.push(parseInt(obj_string[j], 10)-1);       //3 points added to global array
                }                                           // subtract 1 to match face-vertex mapping indices
            }

        }

        //update the eyePt
        x_mid = -(this.minXYZ[0] + this.maxXYZ[0])/2;
        y_mid = -(this.minXYZ[1] + this.maxXYZ[1])/2;
        z_mid = -(this.minXYZ[2] + this.maxXYZ[2])/2;

        // let x_mid = this.maxXYZ[0] + (this.maxXYZ[0] + this.minXYZ[0])/2;
        // let y_mid = this.maxXYZ[1] + (this.maxXYZ[1] + this.minXYZ[1])/2;
        // let z_mid = this.maxXYZ[2] + (this.maxXYZ[2] + this.minXYZ[2])/2;

        mid_dist = Math.sqrt(Math.pow((this.maxXYZ[0] - this.minXYZ[0]), 2) + Math.pow((this.maxXYZ[1] - this.minXYZ[1]), 2) + Math.pow((this.maxXYZ[2] - this.minXYZ[2]), 2));
        // console.log("i'm min_x:", this.minXYZ[0]);
        // console.log("i'm max_x:", this.maxXYZ[0]);
        // console.log("i'm min_y:", this.minXYZ[1]);
        // console.log("i'm max_y:", this.maxXYZ[1]);
        // console.log("i'm min_z:", this.minXYZ[2]);
        // console.log("i'm max_z:", this.maxXYZ[2]);
        // eyePt[0] += x_mid;
        // eyePt[1] += y_mid;
        // eyePt[2] += z_mid; 
   
        // console.log("i'm x:", x_mid);
        // console.log("i'm y:", y_mid);
        // console.log("i'm z:", z_mid);
